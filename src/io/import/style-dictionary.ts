...
type SDNode = Record<string, unknown>;

function isColorLeaf(node: unknown): boolean {
  if (typeof node !== 'object' || node === null) return false;
  const n = node as SDNode;
...
export function parseStyleDictionary(json: string): ImportResult {
  let parsed: unknown;
  try {
  ...
  traverse(parsed, [], tokens, pendingEdges);
